...
const providerConnectionChecks = [
  ...
  },
  {
    ...
    pattern: /provider\s+text\s+not\s+null[\s\S]+account_email\s+text[\s\S]+account_subject\s+text/i,
  },
  {
    label: "provider_connections encrypted token posture",
    pattern: /encrypted_access_token\s+text[\s\S]+encrypted_refresh_token\s+text/i,
  },
  {

...
function assertNoDisabledApprovalButton(): void {
  ...
  const text = readFileSync(uiPath, "utf8");
  if (/disabled\s+type=["']button["'][^>]*>\s*Approve\s*</.test(text) || />\s*Approve\s*<\/button>/.test(text) && /disabled/.test(text)) {
    failOrPending(
      { label: "approve button is wired", path: "src/components/cashflow-cockpit.tsx", mode: "strict" },
      "Approve button still appears disabled in the product UI.",
    );
  } else {
    console.log("ok CP7 UI approval scan: no disabled Approve button pattern found.");
  }
}

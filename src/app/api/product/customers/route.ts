...
import { getProductCustomers } from "../../../../server/repositories/product-customers";
...
export async function GET(request: Request) {
  ...
  try {
    const state = await getProductCustomers({ companyExternalId, caseId });
    ...
    return NextResponse.json(
      {
        status: "error",
        message: error instanceof Error ? error.message : "Unable to load product customers.",
      },
      { status: 500 },
    );
  }

...
import { getProductOverviewState } from "../../../../server/repositories/product-overview";
...
export async function GET(request: Request) {
  ...
  try {
    const overview = await getProductOverviewState({ companyExternalId, caseId });
    ...
    return NextResponse.json(
      {
        status: "error",
        message: error instanceof Error ? error.message : "Unable to load product overview.",
      },
      { status: 500 },
    );
  }
